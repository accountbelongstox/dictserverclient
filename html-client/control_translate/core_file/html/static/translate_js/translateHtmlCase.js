...
class Main {
    ...
    create_groupsHTML(groups_data) {
        ...
        for (let item of groups_data) {
            let group_id = item.id,
                group_name = item.group_n,
                group_language = item.language,
                group_last_time = item.last_time,
                group_count = this.analyze_groupscount(item.link_words, item.count),
                linked_wordids = item.id
                ;
            group_html += `
            <div class="media px-0" data-groupid="${group_id}" data-groupcount="${group_count}"  data-lasttime="${group_last_time}" data-languge="${group_language}" onclick="window.MyDict.event('get_wordsbygroup',this.dataset.groupid)" >
    ...
    create_groupwordsHTML(data, pre_render) {
        ...
        let container_selector = "#wordbox_contents_html"
        let wordTheGroupEles = []
        group_data.forEach((item, index) => {
            let wordelehtml = this.create_translationHTML(pre_render, item, index, project_mode, brief_mode)
            let wordele = $(wordelehtml)
            wordTheGroupEles.push(wordele)
            baseHtmlToolCase.addHtml(container_selector, wordele)
        })
    ...
    create_translationHTML(pre_render, item, index, project_mode, brief_mode) {
        let
            ...
            // read = item.read,
            word_json = item.translate_bing,
            word_index = index + 1,
            last_readtime = this.get_readtimefromtimastamp(item.id)
            ;
    ...
    create_selectorbywid(wid) {
        return `#${this.create_wordid(wid)}`
    }
    ...
    create_wordbox_html(
        ...
        let html = `
        <div class="col-12" id="${contaner_id}" ${this.wordcontaner_token} data-word="${word}" data-index="${word_index}" data-wordid="${id}" ${preload_style}>
    ...
    get_publicgroupmap() {
        return this.public_groupmap
    }

    init_publicgroupmap(group_map) {
        this.public_groupmap = {}
        group_map.forEach(map => {
            this.public_groupmap[map.id] = map
        })
    }

    get_groupmapfromid(wid) {
        let group_map = this.get_publicgroupmap()
        return group_map[wid]
    }

    get_groupmapvaluefromid(wid, key) {
        let map = this.get_groupmapfromid(wid)
        return map[key]
    }

    set_groupmapfromid(wid, set_map) {
        let map = this.get_groupmapfromid(wid)
        for (let key in set_map) {
            map[key] = set_map[key]
        }
        this.public_groupmap[wid] = map
    }

    set_groupmapvaluefromid(wid, key, value) {
        let set_map = {}
        set_map[key] = value
        this.set_groupmapfromid(wid, set_map)
    }
    ...
    get_resolvegroup_words(data) {
        ...
        if (data.length) {
            if (!data[0].is_group_map) {
                for (var i = 0; i < data.length; i++) {
                    let item = data[i]
                    let wid = item.id
                    let group_map = this.get_groupmapfromid(wid)
                    if (group_map) {
                        for (const key in group_map) {
                            let map_value = group_map[key]
                            item[key] = map_value
                        }
                        data[i] = item
                    }
